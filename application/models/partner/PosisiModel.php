...
class PosisiModel extends Render_Model
{
    ...
    public function getIsiPosisiBy($where = [null])
    {
        $this->db->select("a.*, IF(a.status = '2' , 'Tidak Aktif', IF(a.status = '1' , 'Aktif', 'Tidak Diketahui')) as status_str");
        $this->db->from("posisi a");
        $this->db->where($where);
    ...
    public function insert($nama, $no_urut, $status)
    {
        $this->db->trans_start();
        ...
        if ($result) {
            ...
            $data_baru = "Menambah data baru dengan isi nama = $nama, no urut = $no_urut, status = $status";
    ...
    public function update($id, $nama, $no_urut, $status)
    {
        $this->db->trans_start();
        $sebelumnya = $this->getIsiPosisiBy(['id' => $id]);
        $this->db->where('id', $id);
        ...
        if ($result) {
            ...
            $jenis_perubahan = 2;
            $status = ($status == 0 ? $status = 'Dibuat' : ($status == 1 ? $status = 'Aktif' : ($status == 2 ? $status = 'Tidak Aktif' : 'Tidak Diketahui')));
            $data_lama = "Isi data sebelumnya adalah nama = $sebelumnya[nama], no urut = $sebelumnya[no_urut], status = $sebelumnya[status_str]";
            $data_baru = "Mengubah isi data sebelumnya menjadi nama = $nama, no urut = $no_urut, status = $status";
            $this->setActivityLog($modul, $by, $jenis_perubahan, $data_lama, $data_baru);
        }
        $this->db->trans_complete();
        return $result;
