...
class KategorialUmurModel extends Render_Model
{
    ...
    public function getKategorialUmur($id)
    {
        $result = $this->db->get_where("kategorial", ['id' => $id])->row_array();
        return $result;
    }

    public function getIsiKategorialUmurBy($where = [null])
    {
        $this->db->select("a.*, IF(a.status = '2' , 'Tidak Aktif', IF(a.status = '1' , 'Aktif', 'Tidak Diketahui')) as status_str");
        $this->db->from("kategorial a");
        $this->db->where($where);
    ...
    {
        $this->db->trans_start();
        $result = $this->db->insert("kategorial", [
            'nama' => $nama,
            'status' => $status,
            'created_at' => date("Y-m-d H:i:s"),
        ]);
        if ($result) {
            $modul = "Kategorial Umur";
    ...
    {
        $this->db->trans_start();
        $sebelumnya = $this->getIsiKategorialUmurBy(['id' => $id]);
        $this->db->where('id', $id);
        $result = $this->db->update('kategorial', [
            'nama' => $nama,
            'status' => $status,
