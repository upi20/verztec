...
class KepanitiaanModel extends Render_Model
{
    ...
    public function getKepanitiaan($id)
    {
        $result = $this->db->get_where("kepanitiaan", ['id' => $id])->row_array();
        return $result;
    }

    public function getIsiKepanitiaanBy($where = [null])
    {
        $this->db->select("a.*, IF(a.status = '2' , 'Tidak Aktif', IF(a.status = '1' , 'Aktif', 'Tidak Diketahui')) as status_str");
        $this->db->from("kepanitiaan a");
        $this->db->where($where);
    ...
    {
        $this->db->trans_start();
        $result = $this->db->insert("kepanitiaan", [
            'nama' => $nama,
    ...
    {
        $this->db->trans_start();
        $sebelumnya = $this->getIsiKepanitiaanBy(['id' => $id]);
        $this->db->where('id', $id);
        $result = $this->db->update('kepanitiaan', [
            'nama' => $nama,
            'keterangan' => $keterangan,
            'no_urut' => $no_urut,
            'status' => $status,
