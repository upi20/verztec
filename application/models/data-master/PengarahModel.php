...
class PengarahModel extends Render_Model
{
    ...
    public function getPengarah($id)
    {
        $result = $this->db->get_where("pengarah", ['id' => $id])->row_array();
        return $result;
    }

    public function getIsiPengarahBy($where = [null])
    {
        $this->db->select("a.*, IF(a.status = '2' , 'Tidak Aktif', IF(a.status = '1' , 'Aktif', 'Tidak Diketahui')) as status_str");
        $this->db->from("pengarah a");
        $this->db->where($where);
    ...
    {
        $this->db->trans_start();
        $result = $this->db->insert("pengarah", [
            'nama' => $nama,
    ...
    {
        $this->db->trans_start();
        $sebelumnya = $this->getIsiPengarahBy(['id' => $id]);
        $this->db->where('id', $id);
        $result = $this->db->update('pengarah', [
            'nama' => $nama,
            'keterangan' => $keterangan,
            'no_urut' => $no_urut,
            'status' => $status,
