...
class JabatanModel extends Render_Model
{
    ...
    {
        $result = $this->db->get_where("jabatan", ['id' => $id])->row_array();
        return $result;
    }

    public function getIsiJabatanBy($where = [null])
    {
        $this->db->select("a.*, IF(a.status = '2' , 'Tidak Aktif', IF(a.status = '1' , 'Aktif', 'Tidak Diketahui')) as status_str");
        $this->db->from("jabatan a");
        $this->db->where($where);
    ...
    {
        $this->db->trans_start();
        $sebelumnya = $this->getIsiJabatanBy(['id' => $id]);
        $this->db->where('id', $id);
        $result = $this->db->update('jabatan', [
            'nama' => $nama,
            'keterangan' => $keterangan,
            'no_urut' => $no_urut,
            'status' => $status,
