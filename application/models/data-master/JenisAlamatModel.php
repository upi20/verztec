...
class JenisAlamatModel extends Render_Model
{
    ...
    public function getJenisAlamat($id)
    {
        $result = $this->db->get_where("jenis_alamat", ['id' => $id])->row_array();
        return $result;
    }


    public function getIsiJenisAlamatBy($where = [null])
    {
        $this->db->select("a.*, IF(a.status = '0' , 'Tidak Aktif', IF(a.status = '1' , 'Aktif', 'Tidak Diketahui')) as status_str");
        $this->db->from("jenis_alamat a");
        $this->db->where($where);
    ...
    {
        $this->db->trans_start();
        $result = $this->db->insert("jenis_alamat", [
            'nama' => $nama,
    ...
    {
        $this->db->trans_start();
        $sebelumnya = $this->getIsiJenisAlamatBy(['id' => $id]);
        $this->db->where('id', $id);
        $result = $this->db->update('jenis_alamat', [
            'nama' => $nama,
            'keterangan' => $keterangan,
            'status' => $status
        ]);
