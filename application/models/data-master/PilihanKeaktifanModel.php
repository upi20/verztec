...
class PilihanKeaktifanModel extends Render_Model
{
    ...
    public function getPilihanKeaktifan($id)
    {
        $result = $this->db->get_where("keaktifan_pilihan", ['id' => $id])->row_array();
        return $result;
    }

    public function getIsiPilihanKeaktifanBy($where = [null])
    {
        $this->db->select("a.*, IF(a.status = '2' , 'Tidak Aktif', IF(a.status = '1' , 'Aktif', 'Tidak Diketahui')) as status_str");
        $this->db->from("keaktifan_pilihan a");
        $this->db->where($where);
    ...
    {
        $this->db->trans_start();
        $result = $this->db->insert("keaktifan_pilihan", [
            'nama' => $nama,
    ...
    {
        $this->db->trans_start();
        $sebelumnya = $this->getIsiPilihanKeaktifanBy(['id' => $id]);
        $this->db->where('id', $id);
        $result = $this->db->update('keaktifan_pilihan', [
            'nama' => $nama,
            'keterangan' => $keterangan,
            'no_urut' => $no_urut,
            'status' => $status,
