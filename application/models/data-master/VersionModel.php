...
class VersionModel extends Render_Model
{
    ...
    {
        $result = $this->db->get_where("version", ['id' => $id])->row_array();
        return $result;
    }

    public function getIsiVersionBy($where = [null])
    {
        $this->db->select("a.*, IF(a.status = '2' , 'Tidak Aktif', IF(a.status = '1' , 'Aktif', 'Tidak Diketahui')) as status_str");
        $this->db->from("version a");
        $this->db->where($where);
    ...
    public function insert($nama, $keterangan, $tanggal_release, $status)
    {
        $this->db->trans_start();
        $result = $this->db->insert("version", [
            'nama' => $nama,
    ...
    public function update($id, $nama, $keterangan, $tanggal_release, $status)
    {
        $this->db->trans_start();
        $sebelumnya = $this->getIsiVersionBy(['id' => $id]);
        $this->db->where('id', $id);
        $result = $this->db->update('version', [
            'nama' => $nama,
            'keterangan' => $keterangan,
            'tanggal_release' => $tanggal_release,
            'status' => $status,
