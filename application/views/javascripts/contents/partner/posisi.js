...
$(function () {
    function dynamic() {
        ...
        var tableUser = table_html.DataTable({
            "ajax": {
                "url": "<?= base_url()?>partner/posisi/ajax_data/",
                "data": null,
                "type": 'POST'
            },
            "processing": true,
    ...
    $("#btn-tambah").click(() => {
        $("#myModalLabel").text("Tambah List Posisi");
        $('#id').val("");
        $('#nama').val("");
        $('#no_urut').val("");
        $('#status').val("1");
    ...
    $("#form").submit(function (ev) {
        ...
        $.ajax({
            method: 'post',
            url: '<?= base_url() ?>partner/posisi/' + ($("#id").val() == "" ? 'insert' : 'update'),
            data: form,
            cache: false,
            contentType: false,
            processData: false,
    ...
    $("#importExcel").submit(function (ev) {
        ...
        $.ajax({
            method: 'post',
            url: '<?= base_url() ?>partner/posisi/import_excel',
            data: form,
            cache: false,
            contentType: false,
            processData: false,
    ...
    $('#OkCheck').click(() => {
        ...
        $.ajax({
            method: 'post',
            url: '<?= base_url() ?>partner/posisi/delete',
            data: {
                id: id
            }
        }).done((data) => {
...
const Ubah = (id) => {
    ...
    $.ajax({
        method: 'get',
        url: '<?= base_url() ?>partner/posisi/getPosisi',
        data: {
            id: id
        }
    }).done((data) => {
