...
$(function () {
    function dynamic() {
        ...
        var tableUser = table_html.DataTable({
            "ajax": {
                "url": "<?= base_url()?>pengaturan/kategorialUmur/ajax_data/",
                "data": null,
                "type": 'POST'
            },
            "processing": true,
    ...
    $("#btn-tambah").click(() => {
        $("#myModalLabel").text("Form Kategorial Umur");
        $('#id').val("");
        $('#nama').val("");
        $('#status').val("1");
    });
    ...
    $("#form").submit(function (ev) {
        ...
        $.ajax({
            method: 'post',
            url: '<?= base_url() ?>pengaturan/kategorialUmur/' + ($("#id").val() == "" ? 'insert' : 'update'),
            data: form,
            cache: false,
            contentType: false,
            processData: false,
    ...
    $("#importExcel").submit(function (ev) {
        ...
        $.ajax({
            method: 'post',
            url: '<?= base_url() ?>pengaturan/kategorialUmur/import_excel',
            data: form,
            cache: false,
            contentType: false,
            processData: false,
    ...
    $('#OkCheck').click(() => {
        ...
        $.ajax({
            method: 'post',
            url: '<?= base_url() ?>pengaturan/kategorialUmur/delete',
            data: {
                id: id
            }
        }).done((data) => {
...
const Ubah = (id) => {
    ...
    $.ajax({
        method: 'get',
        url: '<?= base_url() ?>pengaturan/kategorialUmur/getKategorialUmur',
        data: {
            id: id
        }
    }).done((data) => {
        if (data.data) {
            data = data.data;
            $("#myModalLabel").text("Ubah Kategorial Umur");
            $('#id').val(data.id);
            $('#nama').val(data.nama);
