...
$(function () {
    function dynamic() {
        ...
        var tableUser = table_html.DataTable({
            "ajax": {
                "url": "<?= base_url()?>data-master/tipeKontak/ajax_data/",
                "data": null,
                "type": 'POST'
            },
            "processing": true,
    ...
    $("#btn-tambah").click(() => {
        $("#myModalLabel").text("Tambah Tipe Kontak");
        $('#id').val("");
        $('#nama').val("");
        $('#keterangan').val("");
        $('#status').val("1");
    ...
    $("#importExcel").submit(function (ev) {
        ...
        $.ajax({
            method: 'post',
            url: '<?= base_url() ?>data-master/tipeKontak/import_excel',
            data: form,
            cache: false,
            contentType: false,
            processData: false,
    ...
    $("#form").submit(function (ev) {
        ...
        $.ajax({
            method: 'post',
            url: '<?= base_url() ?>data-master/tipeKontak/' + ($("#id").val() == "" ? 'insert' : 'update'),
            data: form,
            cache: false,
            contentType: false,
            processData: false,
    ...
    $('#OkCheck').click(() => {
        ...
        $.ajax({
            method: 'post',
            url: '<?= base_url() ?>data-master/tipeKontak/delete',
            data: {
                id: id
            }
        }).done((data) => {
...
const Ubah = (datas) => {
    data = datas.dataset;
    $("#myModalLabel").text("Ubah Tipe Kontak");
    $('#id').val(data.id);
    $('#nama').val(data.nama);
