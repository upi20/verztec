...
$(function () {
    function dynamic() {
        ...
        var tableUser = table_html.DataTable({
            "ajax": {
                "url": "<?= base_url()?>data-master/membership/ajax_data/",
                "data": null,
                "type": 'POST'
            },
            "processing": true,
    ...
    $("#btn-tambah").click(() => {
        $("#myModalLabel").text("Tambah Jenis Membership");
        $('#id').val("");
        $('#nama').val("");
        $('#keterangan').val("");
        $('#status').val("1");
    ...
    $("#form").submit(function (ev) {
        ...
        $.ajax({
            method: 'post',
            url: '<?= base_url() ?>data-master/membership/' + ($("#id").val() == "" ? 'insert' : 'update'),
            data: form,
            cache: false,
            contentType: false,
            processData: false,
    ...
    $("#importExcel").submit(function (ev) {
        ...
        $.ajax({
            method: 'post',
            url: '<?= base_url() ?>data-master/membership/import_excel',
            data: form,
            cache: false,
            contentType: false,
            processData: false,
    ...
    $('#OkCheck').click(() => {
        ...
        $.ajax({
            method: 'post',
            url: '<?= base_url() ?>data-master/membership/delete',
            data: {
                id: id
            }
        }).done((data) => {
...
const Ubah = (id) => {
    ...
    $.ajax({
        method: 'get',
        url: '<?= base_url() ?>data-master/membership/getMembership',
        data: {
            id: id
        }
    }).done((data) => {
        if (data.data) {
            data = data.data;
            $("#myModalLabel").text("Ubah Jenis Membership");
            $('#id').val(data.id);
            $('#nama').val(data.nama);
