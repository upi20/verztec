...
class LaporanHeader extends Render_Controller
{
    public function index()
    {
        // Page Settings
        $this->title = $this->getMenuTitle();
        $this->navigation = ['Data Master', 'Laporan Header'];
        ...
        $this->breadcrumb_3_url = base_url() . 'data-master/laporan-header';

        // content
        $this->content      = 'data-master/laporan-header';
    ...
    public function export_excel()
    {
        ...
        foreach ($details as $detail) {
            ...
            $sheet->setCellValue(chr(65 + ++$c) . "$row", $detail['remark']);
    ...
    public function downloadSample()
    {
        ...
        $col_start = "A";
        $col_end = "I";
        $title_excel = "AuditAny_Data_Master_Company_" . str_replace(" ", "_", $this->getMenuTitle()) . "_Sample";
        // Header excel ================================================================================================
        $spreadsheet = new PhpOffice\PhpSpreadsheet\Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();
        ...
        $spreadsheet->getActiveSheet()->getColumnDimension('B')->setAutoSize(true);
        $spreadsheet->getActiveSheet()->getColumnDimension('C')->setAutoSize(true);
        $spreadsheet->getActiveSheet()->getColumnDimension('D')->setWidth(wh(20));
    ...
    public function import_excel()
    {
        ...
        foreach ($array_from_excel as $val) {
            if ($num > 5 && $val[5]) {
                $resiko = $this->getResikoByName($val[5]);
                $status = strtolower($val[7]);
                if ($resiko == null) {
                    $result = $this->model->insert_resiko($val[5], 1);
                    $resiko_id = $result;
                } else {
                    $resiko_id = $resiko['id'];
                }
                ...
                $tbh = $this->db->insert('client_company', [
                    "nama"   => $val[1] == "" ? NULL : $val[1],
                    "alamat"   => $val[2] == "" ? NULL : $val[2],
                    "no_telepon"  => $val[3] == "" ? NULL : $val[3],
                    "email"  => $val[4] == "" ? NULL : $val[4],
                    "id_resiko"  => $resiko_id,
                    "remark"        => $val[6] == "" ? NULL : $val[6],
                    "status"         => $status_id,
                    "created_at"    => date("Y-m-d H:i:s")
                ]);
            }
    ...
    {
        $result = $this->model->getResikoByName($nama);
        $code = $result ? 200 : 500;
        $this->output_json(["data" => $result], $code);
    }

    public function getStatusByName($nama)
    {
        $result = $this->model->getStatusByName($nama);
        $code = $result ? 200 : 500;
        $this->output_json(["data" => $result], $code);
    }

    public function getLaporanHeader()
    {
        $id = $this->input->get("id");
        $result = $this->model->getLaporanHeader($id);
        $code = $result ? 200 : 500;
        $this->output_json(["data" => $result], $code);
    }
    ...
    public function insert()
    {
        ...
        $result = $this->model->insert($judul, $keterangan_1, $keterangan_2, $status);
        $code = $result ? 200 : 500;
        $this->output_json(["data" => $result], $code);
    }
    ...
    public function update()
    {
        ...
        $keterangan_2 = $this->input->post("keterangan_2");
        $status = $this->input->post("status");
        $result = $this->model->update($id, $judul, $keterangan_1, $keterangan_2, $status);
        $code = $result ? 200 : 500;
        $this->output_json(["data" => $result], $code);
    }
    ...
    function __construct()
    {
        ...
        $this->load->model("data-master/laporanHeaderModel", 'model');
