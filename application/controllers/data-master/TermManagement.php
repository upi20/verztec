...
class TermManagement extends Render_Controller
{
    public function index()
    {
        // Page Settings
        $this->title = $this->getMenuTitle();
        $this->navigation = ['Data Master', 'Term Management'];
        ...
        $this->breadcrumb_3 = 'Term Management';
        $this->breadcrumb_3_url = base_url() . 'data-master/term-management';

        // content
        $this->content      = 'data-master/term-management';
        $this->data['parent']             = $this->model->getDataParent();
    ...
    public function subMenu()
    {
        ...
        $exe = $this->model->getDataChild($menu);
    ...
    public function export_excel()
    {
        ...
        $spreadsheet->getProperties()
            ...
            ->setDescription("List Term Management $date")
    ...
    public function downloadSample()
    {
        ...
        $col_start = "A";
        $col_end = "E";
        $title_excel = "AuditAny_Data_Master_Jenis_Alamat_Sample";
        // Header excel ================================================================================================
        $spreadsheet = new PhpOffice\PhpSpreadsheet\Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();
        ...
        // set width column
        $spreadsheet->getActiveSheet()->getColumnDimension('A')->setAutoSize(true);
        $spreadsheet->getActiveSheet()->getColumnDimension('B')->setWidth(wd(40));
        $spreadsheet->getActiveSheet()->getColumnDimension('C')->setWidth(wd(40));
        $spreadsheet->getActiveSheet()->getColumnDimension('D')->setAutoSize(true);
        $spreadsheet->getActiveSheet()->getColumnDimension('E')->setAutoSize(true);
        // $spreadsheet->getActiveSheet()->getColumnDimension('E')->setWidth(w(30));
        // $spreadsheet->getActiveSheet()->getColumnDimension('F')->setWidth(w(20));
    ...
    public function import_excel()
    {
        ...
        foreach ($array_from_excel as $val) {
            if ($num > 5 && $val[4]) {
                ...
                $tbh = $this->db->insert('jenis_alamat', [
    ...
    public function insert()
    {
        ...
        $this->form_validation->set_rules('keterangan', 'Details', 'trim|required');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $result = $this->model->insert($menu, $sub_menu, $nama, $keterangan, $status);
    ...
    public function update()
    {
        $this->load->library('form_validation');
        $this->form_validation->set_error_delimiters('', '');
        $this->form_validation->set_rules('id', 'id Term Management', 'trim|required|numeric');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $keterangan = $this->input->post('keterangan');
            $status = $this->input->post('status');
            $result = $this->model->update($id, $menu, $sub_menu, $nama, $keterangan, $status);
            ...
            $this->output_json([
                ...
                'length' => 1,
                'data' =>  $result,
                'message' => $status ? 'Data berhasil diubah' : 'Data gagal diubah'
            ], $code);
        }
    }
    ...
    function __construct()
    {
        ...
        $this->load->model("data-master/termManagementModel", 'model');
