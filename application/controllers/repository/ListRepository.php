...
class ListRepository extends Render_Controller
{
    public function index()
    {
        // Page Settings
        $this->title = $this->getMenuTitle();
        $this->navigation = ['Repository', '#'];
        $this->plugins = ['datatables', 'daterangepicker', 'select2', 'moment'];

        // Breadcrumb setting
        $this->breadcrumb_1 = 'Dashboard';
        $this->breadcrumb_1_url = base_url();
        $this->breadcrumb_2 = 'Repository';
        $this->breadcrumb_2_url = '#';
        $this->breadcrumb_3 = $this->getMenuTitle();
        $this->breadcrumb_3_url = base_url() . 'repository/list-repository';

        // content
        $this->data['summary_repository'] = $this->db->get_where('repository', ['status' => 1])->num_rows();
        $this->data['summary_manual'] = $this->db->get_where('repository', ['kategori' => 'MANUAL', 'status' => 1])->num_rows();
        $this->data['summary_p2pk'] = $this->db->get_where('repository', ['kategori' => 'P2PK', 'status' => 1])->num_rows();
        $this->content      = 'repository/list-repository';
    ...
    public function insert()
    {
        ...
        $result = $this->model->insert($kategori, $judul, $keterangan, $status);
        $code = $result ? 200 : 500;
    ...
    public function update()
    {
        ...
        $keterangan = $this->input->post("keterangan");
        $status = $this->input->post("status");
        $result = $this->model->update($id, $kategori, $judul, $keterangan, $status);
        $code = $result ? 200 : 500;
        $this->output_json(["data" => $result], $code);
    }
    ...
    public function ajax_select_list_judul()
    {
        $return = $this->model->list_judul();
        $this->output_json($return);
    }
    ...
    public function export_excel()
    {
        ...
        $start_tabel = $row + 1;
        foreach ($details as $detail) {
            $attachment = base_url() . "files/repository/" . $detail['attachment'];
            $c = 0;
            $row++;
            $tanggal = is_null($detail['updated_at']) || $detail['updated_at'] == '' ?  $detail['created_at'] :  $detail['updated_at'];
            ...
            if ($detail['attachment'] != '') {
                $sheet->setCellValue(chr(65 + ++$c) . "$row", $attachment);
            } else {
    ...
    public function export_pdf()
    {
        ...
        foreach ($details as $i => $detail) {
            ...
            if ($detail['attachment'] != '') {
                $body_table .= "<td><a target='_BLANK' href='{$attachment}'>Download File</a></td>";
            } else {
                $body_table .= "<td></td>";
            }
    ...
    public function import_excel()
    {
        ...
        foreach ($array_from_excel as $val) {
            if ($num > 5 && $val[2]) {
                ...
                $tbh = $this->model->insert($judul, $keterangan, $status);
            }
            $num++;
        }
    ...
    function __construct()
    {
        ...
        $this->load->model("repository/ListRepositoryModel", 'model');
