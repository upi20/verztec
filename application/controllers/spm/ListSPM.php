...
class ListSPM extends Render_Controller
{
    public function index()
    {
        ...
        $this->breadcrumb_2 = 'SPM';
        $this->breadcrumb_2_url = '#';
        $this->breadcrumb_3 = $this->getMenuTitle();
        $this->breadcrumb_3_url = base_url() . 'spm/list-spm';

        // content
        $this->data['summary_spm'] = $this->db->get_where('spm')->num_rows();
        $this->data['summary_trainer'] = $this->db->group_by('id_trainer')->get_where('spm')->num_rows();
        $this->data['summary_status_aktif'] = $this->db->get_where('spm', ['status' => 1])->num_rows();
        $this->data['summary_status_tidak'] = $this->db->get_where('spm', ['status' => 2])->num_rows();
        $this->content      = 'spm/list-spm';
    ...
    public function insert()
    {
        ...
        $result = $this->model->insert($judul, $trainer, $status);
        $code = $result ? 200 : 500;
    ...
    public function update()
    {
        ...
        $trainer = $this->input->post("trainer");
        $status = $this->input->post("status");
        $result = $this->model->update($id, $judul, $trainer, $status);
        $code = $result ? 200 : 500;
        $this->output_json(["data" => $result], $code);
    }
    ...
    public function ajax_select_list_profile()
    {
        $return = $this->model->list_profile();
        $this->output_json($return);
    }

    public function ajax_select_list_trainer()
    {
        $return = $this->model->list_trainer();
        $this->output_json($return);
    }
    ...
    public function downloadSample()
    {
        ...
        $col_start = "A";
        $col_end = "C";
        $title_excel = "AuditAny_" . str_replace(" ", "_", $this->getMenuTitle() . "_Sample");
        // Header excel ================================================================================================
    ...
    public function export_excel()
    {
        ...
        $trainer = $this->input->get('filter-trainer');
        $filter = [
            'date' => [
                'start' => $date_start,
                'end' => $date_end,
        ...
        $start_tabel = $row + 1;
        foreach ($details as $detail) {
            $bahan = base_url() . "files/spm/" . $detail['bahan'];
            $absensi = base_url() . "files/spm/" . $detail['absensi'];
            $c = 0;
            $row++;
            $tanggal = is_null($detail['updated_at']) || $detail['updated_at'] == '' ?  $detail['created_at'] :  $detail['updated_at'];
            $tanggal = date("d-m-Y", strtotime($tanggal));
            ...
            $sheet->setCellValue(chr(65 + ++$c) . "$row", $detail['trainer']);
            if ($detail['bahan'] != '') {
                $sheet->setCellValue(chr(65 + ++$c) . "$row", $bahan);
            } else {
                $sheet->setCellValue(chr(65 + ++$c) . "$row", '');
            }
            if ($detail['absensi'] != '') {
                $sheet->setCellValue(chr(65 + ++$c) . "$row", $absensi);
            } else {
                $sheet->setCellValue(chr(65 + ++$c) . "$row", '');
            }
        ...
        $spreadsheet->getActiveSheet()->getColumnDimension('B')->setAutoSize(true);
        $spreadsheet->getActiveSheet()->getColumnDimension('C')->setAutoSize(true);
        $spreadsheet->getActiveSheet()->getColumnDimension('D')->setWidth(w(60));
        $spreadsheet->getActiveSheet()->getColumnDimension('E')->setWidth(w(60));
    ...
    public function export_pdf()
    {
        ...
        foreach ($details as $i => $detail) {
            ...
            $body_table .= "<td>{$detail['trainer']}</td>";
            if ($detail['bahan'] != '') {
                $body_table .= "<td><a target='_BLANK' href='{$bahan}'>Download File</a></td>";
            } else {
                $body_table .= "<td></td>";
            }
            if ($detail['absensi'] != '') {
                $body_table .= "<td><a target='_BLANK' href='{$absensi}'>Download File</a></td>";
            } else {
                $body_table .= "<td></td>";
            }
    ...
    function __construct()
    {
        ...
        $this->load->model("spm/ListSPMModel", 'model');
